...
CREATE TABLE `dropDownloads` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_user_drop` (`postId`, `userId`) COMMENT 'One download per user per drop',
  KEY `idx_postId` (`postId`),
  KEY `idx_userId` (`userId`),
  CONSTRAINT `fk_downloads_drop` FOREIGN KEY (`postId`) REFERENCES `drops` (`id`) ON DELETE CASCADE,
