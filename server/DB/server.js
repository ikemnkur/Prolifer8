...
server.post(PROXY + '/api/auth/verification-docs/:username', authenticateToken, async (req, res) => {
  ...
  busboy.on('file', (fieldname, file, info) => {
    ...
    writeStream.on('finish', async () => {
      savedFiles[fieldname] = localPath;
      pendingWrites--;
...
// Prolifer8 burn-rate decay — every ENGINE_K minutes (default 5)
...
async function upsertStripeTransactionRecord(record) {
  await knex.raw(
    `INSERT INTO stripeTransactions (
      stripePaymentIntentId, stripeChargeId, stripeCheckoutSessionId, stripeCustomerId,
      stripeInvoiceId, stripeSubscriptionId, status, amount, amountReceived, currency,
      paymentMethodTypes, description, receiptEmail, customerEmail, customerName,
      livemode, metadata, rawPayload, stripeCreatedAt, syncedAt
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE
...
async function syncStripeTransactionsCron(limit = 100) {
  ...
  const createdSinceUnix = Math.floor(Date.now() / 1000) - ((parseInt(process.env.STRIPE_SYNC_HOURS || '720', 10)) * 60 * 60);
  const paymentIntents = await stripe.paymentIntents.list({
    limit: Math.min(parseInt(limit, 10) || 100, 100),
    created: { gte: createdSinceUnix },
    expand: ['data.customer', 'data.latest_charge']
  });
  ...
  console.log(`💳 Stripe sync complete: ${inserted} inserted, ${updated} updated, ${paymentIntents.data.length} scanned.`);
  return {
    success: true,
    inserted,
    updated,
    scanned: paymentIntents.data.length,
  };
}
