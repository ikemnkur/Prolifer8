...
module.exports = function createAdminRouter(deps = {}) {
  ...
  function adminLayout(title, activeNav, bodyContent) {
    const navItems = [
      ...
      { href: '/review/posts', icon: '🔥', label: 'Drops' },
...
<body>
  ...
  router.get('/review/posts', async (req, res) => {
    try {
      ...
      const statusTabs = ['', 'pending', 'draft', 'active', 'dropped', 'expired', 'removed']
        .map(s => {
          ...
          return `<a href="{{BASE}}/review/posts${qs({ status: s, page: 1 })}"
      ...
      const typeTabs = FILE_TYPES.map(t => {
        ...
        return `<a href="{{BASE}}/review/posts${qs({ type: t, page: 1 })}"
          class="btn ${active ? 'btn-primary' : 'btn-outline'}"
      ...
      const thSort = (col, label) => {
        ...
        return `<th><a href="{{BASE}}/review/posts${qs({ sort: col, dir: nextDir, page: 1 })}"
      ...
      const paginationLinks = (() => {
        ...
        const mkLink = (p, label, disabled = false) => disabled
          ...
          : `<a href="{{BASE}}/review/posts${qs({ page: p })}" style="padding:4px 10px;border-radius:6px;background:var(--surface2);color:var(--text);font-size:0.85em;text-decoration:none;">${label}</a>`;
      ...
      const rows = drops.map(d => {
        ...
        return `<tr>
          <td><a href="{{BASE}}/review/post/${escapeHtml(d.id)}" style="color:var(--accent);text-decoration:none;font-family:monospace;font-size:0.8em;">${escapeHtml(d.id.slice(0, 8))}…</a></td>
          <td><a href="{{BASE}}/review/post/${escapeHtml(d.id)}" style="color:var(--text);text-decoration:none;font-weight:600;">${escapeHtml(d.title || 'Untitled')}</a></td>
      ...
      const body = `
        ...
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:12px;">
          ...
          <div style="flex:1;min-width:220px;display:flex;gap:6px;">
            ...
            ${search ? `<a href="{{BASE}}/review/posts${qs({ search: '', page: 1 })}" class="btn btn-outline" style="font-size:0.8em;padding:5px 10px;" title="Clear search">✕</a>` : ''}
          </div>
      ...
      render(req, res, 'Drop Review', '/review/posts', body);
    } catch (err) {
      render(req, res, 'Error', '/review/posts',
  ...
  router.get('/review/post/:id', async (req, res) => {
    try {
      ...
      if (!drop) {
        return render(req, res, 'Not Found', '/review/posts',
          `<div class="card"><h3 style="color:var(--red);">Drop not found</h3><p>No drop with ID ${escapeHtml(req.params.id)}</p>
           <a href="{{BASE}}/review/posts" class="btn btn-outline" style="margin-top:12px;">← Back to drops</a></div>`);
      }
      ...
      const body = `
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
          <a href="{{BASE}}/review/posts" style="color:var(--text2);text-decoration:none;font-size:1.2em;">←</a>
        ...
        <script>
          async function dropAction(action) {
            ...
            try {
              const res = await fetch('{{BASE}}/api/review/post/${escapeHtml(drop.id)}/' + action, { method: 'POST' });
              const data = await res.json();
              if (data.ok) {
                location.reload();
              } else {
      ...
      render(req, res, drop.title || 'Drop Detail', '/review/posts', body);
    } catch (err) {
      render(req, res, 'Error', '/review/posts',
  ...
  router.post('/api/review/post/:id/approve', express.json(), async (req, res) => {
    try {
  ...
  router.post('/api/review/post/:id/reject', express.json(), async (req, res) => {
    try {
      const updated = await knex('drops').where({ id: req.params.id, status: 'pending' })
        .update({ status: 'draft' });
      if (!updated) return res.json({ ok: false, error: 'Drop not found or not in pending status' });
  ...
  router.post('/api/review/post/:id/remove', express.json(), async (req, res) => {
    try {
